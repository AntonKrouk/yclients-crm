...
// Комментарий записи YClients и её род: 'visit' — визит, 'birthday' — запись-напоминание о ДР
// (без услуг, фиолетовая, «др» в комментарии). Нужны, чтобы прошедшие записи без отметки
// посещаемости отличать: визит бесплатного клиента считаем посещением, напоминание — нет.
ensureColumn('visits', 'comment', 'TEXT');
ensureColumn('visits', 'kind', 'TEXT');
// «Ходит, но не платит» — вычисляется каждым синком (sync.recomputeFreeClients), такие люди
// в автоматический обзвон не попадают.
ensureColumn('clients', 'free_client', 'INTEGER DEFAULT 0');

// Момент СОЗДАНИЯ записи в YClients (create_date) — не путать с датой самого визита.
// Нужен, чтобы понять, записался ли человек ПОСЛЕ звонка: по дате визита это не видно.
// У записей, приехавших до появления колонки, поле пустое — там ориентируемся на дату визита.
ensureColumn('visits', 'booked_at', 'TEXT');
// Звонок, после которого человек записался в оговорённое окно (21 день). Считается
// пересчётом после каждого синка, а не в момент отметки: запись появляется позже разговора.
ensureColumn('task_actions', 'auto_booked', 'INTEGER DEFAULT 0');
