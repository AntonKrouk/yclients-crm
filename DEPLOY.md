...
### Бэкап базы (настроено 20.08.2026)

Три независимых уровня:
1. **Локальные копии** — `/opt/yclients-crm/data/backup-ГГГГ-ММ-ДД.db`, хранятся 30 дней.
2. **Снапшоты виртуалки** — аварийное копирование в панели Timeweb.
3. **Копии вне сервера** — Yandex Object Storage, бакет `prive7-crm-backup` (закрытый,
   правило жизненного цикла удаляет объекты старше 30 дней).

Всё делает `deploy/backup.sh`, он же стоит в кроне на 3:30. Ключи S3 лежат в `/root/.s3cred`
(права 600), сервисный аккаунт `crm-backup` имеет роль `storage.uploader` — **только запись**.
Это намеренно: получив доступ к серверу, стереть архив в облаке нельзя.

Почему `VACUUM INTO`, а не `cp`: SQLite работает в режиме WAL, свежие транзакции лежат в
отдельном файле `crm.db-wal` и переносятся в основной не сразу. Простое копирование
`crm.db` во время активной записи даёт копию без них, а при неудачном стечении — рваный
файл. `VACUUM INTO` делает целостный снимок на живой базе.

### Восстановление из бэкапа

Скачать нужную копию из бакета через консоль Yandex (у ключа сервера прав на чтение нет
намеренно). Дальше:

```bash
systemctl stop yclients-crm
cd /opt/yclients-crm/data
mv crm.db crm.db.broken                    # старую не удаляем, а отодвигаем
gunzip -c /путь/к/crm-ГГГГ-ММ-ДД.db.gz > crm.db
rm -f crm.db-wal crm.db-shm                # хвосты старой базы, с новым файлом несовместимы
chown crm:crm crm.db
systemctl start yclients-crm
```

Если восстановились не из того дня — вернуть `crm.db.broken` на место и повторить.
