...
// ВНИМАНИЕ. Разницу между этими двумя строками считать эффектом программы НЕЛЬЗЯ.
// Проверка на боевой базе показала: в группе «не звонили» все до одного со статусом
// dismissed — движок снимает задачу, когда клиент записался САМ (а также по VIP,
// «не беспокоить» и паузе после звонка). То есть эта группа целиком состоит из тех,
// кто вернулся своим ходом, и сравнение с ней перевёрнуто по построению.
// Строки оставлены как диагностика, но вывод по ним не делаем.
if (A.people && B.people) {
  const lift = ((A.returned / A.people) - (B.returned / B.people)) * 100;
  console.log(`\nФормальная разница: ${lift.toFixed(1)} п.п. — НО ЭТО НЕ ЭФФЕКТ ПРОГРАММЫ.`);
  console.log('Группа «не звонили» собрана не случайно: туда попадают те, у кого задачу');
  console.log('сняли, а снимают её как раз при самостоятельной записи. Сравнивать нельзя.');
  console.log('Честный ответ даст только эксперимент: часть задач случайно помечать');
  console.log('«не звонить» и через месяц сравнить группы.');
}
